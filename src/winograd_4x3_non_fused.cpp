...
template <typename inoutT, 
          typename calcT, 
          int work_group_size>
__global__ static void input_transform_filter_transform
                                    ...
                                    size_t                 div)
{
  ...
  if(blx < div) 
  {
    ...
    for (int i = 0; i < TILE_IN_H; ++i) {
      ...
      U[i * TILE_IN_W * simdDimSize1 + 4 * simdDimSize1 + idx] = z4;
      U[i * TILE_IN_W * simdDimSize1 + 5 * simdDimSize1 + idx] = z5;
    }
  }
  else 
  {
    ...
    for (int h = 0; h < TILE_IN_H; ++h) {
      ...
      V[h * TILE_IN_W * simdDimSize2 + 4 * simdDimSize2 + idx] = z4;
      V[h * TILE_IN_W * simdDimSize2 + 5 * simdDimSize2 + idx] = z5;
    }
  }
